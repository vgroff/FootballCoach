...
The logged entropy and per-head breakdown must stay UNBOOSTED, the boost must be exactly (w - 1) x P(exec_move) x the raw
...
def _restore(trainer):
    # ent_kick_power_only_weight/ent_kick_dir_azimuth_only_weight/ent_kick_dir_z_only_weight come
    # from the live config (non-1.0 since run 307/308) and would otherwise leak extra boost terms
    # into every assertion here, which only cares about ent_kick_weight/ent_move_dir_weight -- this
    # file doesn't test those others at all.
...
def test_boost_is_w_minus_one_times_the_move_dir_term(trainer):
    ...
    # bk["move_dir"] is already P(exec_move) x raw entropy, so the boost is (w - 1) x that.
...
def test_boost_only_reaches_move_dir_log_kappa(trainer):
    ...
    # boost = (w - 1) x P(exec_move) x H(kappa) and H decreases as kappa grows, so d boost / d log kappa < 0: the trainer
